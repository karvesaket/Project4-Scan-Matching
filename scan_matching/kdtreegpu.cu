...
__device__ void buildKdTreeRecursive(glm::vec4* points, int beg, int end, int align, int root, glm::vec4* tree) {
	...
	if (align == 0) {
		thrust::sort(thrust::device, thrust_points + beg, thrust_points + end, XComp());
	}
	else if (align == 1) {
	...
	buildKdTreeRecursive(points, beg, mid, (align + 1)%3, 2*root + 1, tree);
...
__global__ void getKdTree(glm::vec4* tree, glm::vec4* points, int num) {
	buildKdTreeRecursive(points, 0, num, 0, 0, tree);
}
...
namespace KDTreeGPU {
	...
	void buildTree(float* points, int num) {
		...
		cudaMalloc((void**)&dev_y_vec3, (num/3) * sizeof(glm::vec4));
		checkCUDAErrorWithLine("cudaMalloc dev_y failed!");
		dim3 numBlocks(((num/3) + blockSize - 1) / blockSize);
		convertToVec3 << <numBlocks, blockSize >> > (points, dev_y_vec3, num/3);
		
		//Call recursive device function to build tree
		int treeSize = 1 << ilog2ceil(num / 3);
		cudaMalloc((void**)&dev_tree, treeSize * sizeof(glm::vec4));
		checkCUDAErrorWithLine("cudaMalloc dev_tree failed!");
		getKdTree << <1, 1 >> > (dev_tree, dev_y_vec3, num/3);
